...
class BackendDoc {
  ...
  getChanges(haveDeps) {
    ...
    for (let hash of haveDeps) {
      ...
      //const successors = this.dependentsByHash[hash] // comentei Fabio Bosisio
      //if (!successors) throw new RangeError(`hash not found: ${hash}`) // comentei Fabio Bosisio
      //stack.push(...successors) // comentei Fabio Bosisio
    }
    ...
    while (stack.length > 0) {
      const hash = stack.pop()
      if (!seenHashes[hash]) {
      //  const deps = this.dependenciesByHash[hash] // comentei Fabio Bosisio
      //  if (!deps) throw new RangeError(`hash not found: ${hash}`) // comentei Fabio Bosisio
      //  stack.push(...deps) // comentei Fabio Bosisio
        seenHashes[hash] = true
      }
    }
