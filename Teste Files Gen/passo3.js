...
console.log("Novo Documento do passo 3\n ", old);
...
currentDoc3 = Automerge.change(Doc3, 'Adiciona mais um card', doc => {
  doc.cards.push({ title: 'P2P does not scale!', done: false })
})
console.log("Adiciona mais um card\n ",currentDoc3);

let changes = Automerge.getChanges(Doc3, currentDoc3);
console.log("\nObtem a diferenca entre o documento do passo 2 e o do passo 3\n ");
...
console.log("Salva a diferenca entre o passo 2 e 3 em disco!\n ");

// Salva o arquivo local .automerge com os metadados automerge do json
fs.writeFileSync("file3.automerge", Automerge.save(currentDoc3), {encoding: null}); 
		
// Salva o arquivo local .json
fs.writeFileSync("file3.json", JSON.stringify(currentDoc3), {encoding: null}); 
